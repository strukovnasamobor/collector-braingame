...
export function AuthProvider({ children }) {
  ...
  // Native (Capacitor / Android) Google sign-in: invokes the OS-level
  // Google account chooser via the native Google Sign-In SDK, then hands
  // the returned ID token to Firebase Auth via signInWithCredential. The
  // Firebase user produced is identical to the web flow, so everything
  // downstream (onAuthStateChanged, ensurePlayerProfile, ID-token APIs)
  // works unchanged. Used instead of One Tap / popup because both rely
  // on FedCM or popup APIs that are disabled / broken inside WebView.
  const signInWithNativeGoogle = useCallback(async () => {
    ...
    try {
      ...
      if (!idToken) {
        setError('Native sign-in returned no ID token.');
        return;
      }
      const cred = GoogleAuthProvider.credential(idToken);
      const userCredential = await signInWithCredential(auth, cred);
      if (!isAllowedEmail(userCredential.user?.email)) {
        try { await FirebaseAuthentication.signOut(); } catch (_) {}
        await fbSignOut(auth);
        setError('Only @gmail.com accounts can sign in.');
      }
    } catch (e) {
      // The plugin throws on user-cancel as well as on real errors. Swallow
      // cancellations silently, surface anything else.
      const code = e?.code || e?.message || '';
      if (typeof code === 'string' && /cancel/i.test(code)) return;
  ...
  }, [promptOneTap, signInWithPopupFlow, signInWithNativeGoogle]);

  const signOut = useCallback(async () => {
    await fbSignOut(auth);
    if (IS_NATIVE) {
      // Clears the Google account session held by the native SDK so the
      // next sign-in shows the account chooser instead of silently picking
